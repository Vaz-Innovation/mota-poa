## Plan: Mota POA Next.js + WordPress GraphQL Migration

Migrate mota-poa from React+Vite SPA to Next.js Pages Router, adopting the GraphQL architecture proven in manuel-antunes and vazinnovation (codegen + typed documents + gqlpc + execute + fragments-as-props), while removing Supabase/Auth and using WordPress as the single source of truth. The rollout is a clean in-place rewrite with build-gated checkpoints (`gen` then `build`) to satisfy acceptance criteria.

**Steps**

1. Phase 1: Baseline and dependency transition (_blocks all later steps_)
...
2. Phase 2: GraphQL framework bootstrap (_depends on 1; can run partly parallel with Phase 3 query authoring_)
...
3. Phase 3: WordPress query/mutation contract design (_depends on 2; parallel with component refactor prep in Phase 4_)
...
4. Phase 4: Route migration to Next Pages Router (_depends on 1 and 2_)
...
5. Phase 5: Component architecture refactor (atomic design by responsibility, no atoms/molecules folders) (_depends on 3; mostly parallel with 4 after GraphQL contracts exist_)
...
- component props use `FragmentType<typeof ...>` + `useFragment(...)`
   5.3 Replace standard `<img>` tags with Next.js `<Image />` components for improved performance and optimized loading strategies. Ensure remote images are allowed in `next.config.js`.
   5.4 Keep folder naming pragmatic (feature-oriented), avoiding forced `atoms/molecules` hierarchy while still eliminating code smell from oversized components.

6. Phase 6: Supabase/Auth removal and WordPress source-of-truth enforcement (_depends on 4_)
...
7. Phase 7: Build hardening and acceptance validation (_depends on all prior phases_)
   ...
   7.6 Perform a full SEO audit: Verify page titles, meta descriptions, Open Graph (OG) tags, and Schema.org structured data for all key routes.

8. Phase 8: SEO & Metadata Optimization (_depends on 4_)
   8.1 Implement a centralized SEO component or helper to manage `<Head>` metadata consistently.
   8.2 Ensure dynamic Open Graph tags (og:title, og:description, og:image) are populated for blog posts.
   8.3 Add JSON-LD Structured Data (BlogPosting, FAQ, Search) to improve SERP appearance.
   8.4 Implement canonical URLs and lang alternates to prevent duplicate content issues.

9. Phase 9: Multi-Language Strategy & URL Localization (_depends on 4 and 8_)
   9.1 Enable Next.js `i18n` in `next.config.js` with supported locales.
   9.2 Synchronize `LanguageContext` with the Next.js router locale.
   9.3 Implement `localePathOverrides` in `Header` and `LanguageSelector`.
   9.4 Ensure `getStaticPaths` generates routes for all supported locales.
   9.5 Integrate alternate language links (`hreflang`) in the `SEO` component.



- `/Users/manuelantunes/Projects/Vas/mota-poa/package.json` — replace Vite scripts/deps; add Next + GraphQL codegen scripts/deps.
- `/Users/manuelantunes/Projects/Vas/mota-poa/codegen.ts` — retain and adapt schema/doc generation contract for Next structure.
- `/Users/manuelantunes/Projects/Vas/mota-poa/src/App.tsx` — decommission React Router app root.
- `/Users/manuelantunes/Projects/Vas/mota-poa/src/main.tsx` — remove Vite entrypoint responsibilities.
- `/Users/manuelantunes/Projects/Vas/mota-poa/src/pages/Blog.tsx` — migrate to Next page/query model.
- `/Users/manuelantunes/Projects/Vas/mota-poa/src/pages/BlogPost.tsx` — migrate to slug page with static paths/props + fragment-based rendering.
- `/Users/manuelantunes/Projects/Vas/mota-poa/src/pages/Index.tsx` — migrate to Next homepage route.
- `/Users/manuelantunes/Projects/Vas/mota-poa/src/pages/NotFound.tsx` — migrate into Next 404 handling.
- `/Users/manuelantunes/Projects/Vas/mota-poa/src/pages/AdminBlog.tsx` — remove in favor of WordPress admin workflow.
- `/Users/manuelantunes/Projects/Vas/mota-poa/src/pages/Auth.tsx` — remove (auth features removed).
- `/Users/manuelantunes/Projects/Vas/mota-poa/src/components/ProtectedRoute.tsx` — remove.
- `/Users/manuelantunes/Projects/Vas/mota-poa/src/hooks/useAuth.tsx` — remove.
- `/Users/manuelantunes/Projects/Vas/mota-poa/src/integrations/supabase/client.ts` — remove.
- `/Users/manuelantunes/Projects/Vas/mota-poa/src/integrations/supabase/types.ts` — remove.
- `/Users/manuelantunes/Projects/Vas/mota-poa/supabase/functions/import-article-url/index.ts` — remove/deprecate from app dependency path.
- `/Users/manuelantunes/Projects/Vas/mota-poa/supabase/functions/rewrite-article/index.ts` — remove/deprecate from app dependency path.
...
- Locale launch: Multi-language enabled from start (`pt-BR` default, with full URL localization and `i18n` routing). Mapping architecture ready for EN/PT content fetching from WordPress.
